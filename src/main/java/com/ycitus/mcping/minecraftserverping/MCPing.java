package com.ycitus.mcping.minecraftserverping;
    ...
    public static MCPing getMotd(String host, int port, int timeout) throws IOException {
        ...
        if (data_json.containsKey("players")) {
            if (data_json.get("players") instanceof JSONObject) {
                ...
                if (players_json.get("sample") instanceof JSONArray) {
                    JSONArray player_sample_json = players_json.getJSONArray("sample");
                    if (player_sample_json.size() > 0){
                        for (int i = 0; i < online_players; i++) {
                            playerList.add(player_sample_json.getJSONObject(i).getString("name"));
                        }
                    }
                }
            }
