...
public class IsShowPlayerCommand extends RobotCommand {
    public IsShowPlayerCommand(String rule) {
        super(rule);
    ...
    public void runCommand(int msgType, int time, long fromGroup, long fromQQ, MessageChain messageChain) {
        ...
        if (strings.length == 2){
            if (strings[1].equals("true")){
                FileManager.applicationConfig_File.getSpecificDataInstance().Server.isShowPlayer = true;
                FileManager.applicationConfig_File.saveFile();
                FileManager.applicationConfig_File.reloadFile();
                MessageManager.sendMessageBySituation(fromGroup, fromQQ, "服务器玩家列表显示开启成功");
            } else if (strings[1].equals("false")){
                FileManager.applicationConfig_File.getSpecificDataInstance().Server.isShowPlayer = false;
                FileManager.applicationConfig_File.saveFile();
                FileManager.applicationConfig_File.reloadFile();
                MessageManager.sendMessageBySituation(fromGroup, fromQQ, "服务器玩家列表显示关闭成功");
            }
        }
    }
