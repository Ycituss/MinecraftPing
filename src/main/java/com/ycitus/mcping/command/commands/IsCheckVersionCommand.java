...
public class IsCheckVersionCommand extends RobotCommand {
    public IsCheckVersionCommand(String rule) {
        super(rule);
    ...
    public void runCommand(int msgType, int time, long fromGroup, long fromQQ, MessageChain messageChain) {
        ...
        if (strings.length == 2){
            if (strings[1].equals("true")){
                FileManager.applicationConfig_File.getSpecificDataInstance().Server.isCheckVersion = true;
                FileManager.applicationConfig_File.saveFile();
                FileManager.applicationConfig_File.reloadFile();
                MessageManager.sendMessageBySituation(fromGroup, fromQQ, "mc版本检测开启成功");
            } else if (strings[1].equals("false")){
                FileManager.applicationConfig_File.getSpecificDataInstance().Server.isCheckVersion = false;
                FileManager.applicationConfig_File.saveFile();
                FileManager.applicationConfig_File.reloadFile();
                MessageManager.sendMessageBySituation(fromGroup, fromQQ, "mc版本检测关闭成功");
            }
        }
    }
}
